      ...
      {Array.from({ length: rows }).map((_, i) => (
        <li key={i} className="flex gap-4 px-5 py-4">
          ...
          <div className="flex-1 space-y-2 py-0.5">
            <div className="h-4 w-2/5 animate-pulse rounded bg-slate-100 dark:bg-slate-700" />
            <div className="h-3 w-3/5 animate-pulse rounded bg-slate-100 dark:bg-slate-700" />
          </div>
          <div className="mt-1 h-3 w-14 animate-pulse rounded bg-slate-100 dark:bg-slate-700" />
        </li>
      ))}
    </>
  )
