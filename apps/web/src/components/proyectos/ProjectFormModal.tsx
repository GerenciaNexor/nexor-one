            ...
            {type === 'limite' && (
              <div className="rounded-lg border border-slate-200 p-3 dark:border-slate-700">
                ...
                <div className="space-y-2">
                  <select value={alertMode} onChange={(e) => setAlertMode(e.target.value as 'amount' | 'pct')} className={inp}>
                    <option value="amount">Por monto ($)</option>
                    <option value="pct">Por porcentaje (%)</option>
                  </select>
                  {alertMode === 'amount' ? (
                    <MoneyInput value={alertValue} onChange={setAlertValue} className={inp} placeholder="Ej: 18.000.000" />
                  ) : (
                    <input type="number" min="1" max="100" step="1" value={alertValue} onChange={(e) => setAlertValue(e.target.value)} className={inp} placeholder="Ej: 90" />
                  )}
                </div>
                <p className="mt-1 text-[11px] text-slate-400">Avisar al acercarse al límite (p. ej. al llegar al 90% o a $18.000.000).</p>
                <div className="mt-3">
