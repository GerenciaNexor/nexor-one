  ...
  return (
    <div className="p-6">
      <header className="mb-6">
        <h1 className="text-2xl font-semibold text-slate-900 dark:text-slate-100">Suscripciones</h1>
                    ...
                    <tr key={t.id} className="border-t border-slate-100 hover:bg-slate-50 dark:border-white/5 dark:hover:bg-white/5">
                      ...
                      <td className="px-5 py-3.5">
                        {t.subscription && t.subscription.amount > 0 ? (
                          <span className="text-slate-700 dark:text-slate-200">{fmtMoney(t.subscription.amount, t.subscription.currency)}<span className="text-xs text-slate-500">/mes</span></span>
                        ) : (
                          <span className="text-slate-500">Sin definir</span>
                        )}
                      </td>
                      <td className="px-5 py-3.5 text-xs text-slate-500 dark:text-slate-400">{fmtDate(t.createdAt)}</td>
                      <td className="px-5 py-3.5 text-right">
                        <div className="flex justify-end gap-2">
                          <button
                            onClick={() => setAmountTarget(t)}
                            className="rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-xs font-medium text-slate-700 transition-colors hover:bg-slate-100 dark:border-white/10 dark:bg-white/5 dark:text-slate-200 dark:hover:bg-white/10"
                          >
                            Editar monto
                          </button>
                          <button
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-6 text-slate-700 shadow-2xl dark:border-white/10 dark:bg-[#12162a] dark:text-slate-200">
            ...
            <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
              ...
              <span className="font-medium text-slate-700 dark:text-slate-200">{target.name}</span>.
            </p>
            ...
            <textarea
              ...
              className="mt-1 w-full resize-none rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-violet-500 dark:border-white/10 dark:bg-white/5 dark:text-slate-200 dark:placeholder:text-slate-600"
            />
            <div className="mt-1 flex justify-between text-xs text-slate-500">
              <span>{modalErr && <span className="text-red-600 dark:text-red-400">{modalErr}</span>}</span>
            ...
            <div className="mt-5 flex justify-end gap-2">
              <button
                onClick={closeModal}
                disabled={saving}
                className="rounded-lg border border-slate-200 px-4 py-2 text-sm text-slate-700 hover:bg-slate-50 disabled:opacity-50 dark:border-white/10 dark:text-slate-300 dark:hover:bg-white/5"
              >
                Cancelar
              </button>
              <button
...
function EditAmountModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
      <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-6 text-slate-700 shadow-2xl dark:border-white/10 dark:bg-[#12162a] dark:text-slate-200">
        <h3 className="text-lg font-semibold text-slate-900 dark:text-slate-100">Editar monto mensual</h3>
        <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
          Suscripción de <span className="font-medium text-slate-700 dark:text-slate-200">{tenant.name}</span>.
        </p>
        ...
        <label className="mt-4 block text-xs font-medium text-slate-500 dark:text-slate-400">Motivo <span className="text-red-600 dark:text-red-400">*</span></label>
        <textarea
          value={reason} onChange={(e) => setReason(e.target.value)} rows={3} maxLength={500}
          placeholder="Explica por qué (queda auditado)…"
          className="mt-1 w-full resize-none rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 placeholder:text-slate-400 outline-none focus:ring-2 focus:ring-violet-500/60 dark:border-white/10 dark:bg-white/5 dark:text-slate-200 dark:placeholder:text-slate-600"
        />
