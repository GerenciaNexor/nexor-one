      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex w-full sm:w-auto items-center gap-2">
          <input
            ...
            placeholder="Buscar…"
            className="min-w-0 flex-1 sm:w-64 rounded-lg border border-slate-200 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none"
          />
          <button
      ...
      <div className="mt-5 sm:hidden overflow-hidden rounded-xl border border-slate-200 bg-white divide-y divide-slate-100">
        {loading ? (
          ...
          filtered.map((u) => {
            ...
            return (
              <div key={u.id} className={`px-4 py-4 ${u.isActive ? '' : 'opacity-50'}`}>
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <div className="mt-2 flex flex-wrap items-center gap-1.5">
                      ...
                      {u.module && (
                        <span className="text-xs text-slate-400">{MODULE_LABELS[u.module]}</span>
                      )}
                    </div>
                    {u.branch && (
                      <p className="mt-1 text-xs text-slate-500">{u.branch.name}</p>
                    )}
                  </div>
